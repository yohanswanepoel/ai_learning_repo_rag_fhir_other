...
def npi_to_property_str(id):
    ...
    return f'{"identifier_0_value": "{id}", "identifier_0_system": "http://hl7.org/fhir/sid/us-npi"}'
...
def resource_to_string(resource):
    resource_type = resource['resourceType']
    fhir_resource_dict = flatten_fhir(resource)
    fhir_string_array = FHIR_to_string(resource)
    fhir_resource_name = resource_name(resource)
    fhir_str = ""
    if fhir_string_array is not None:
        fhir_str = ' '.join(fhir_string_array)
    fhir_resource_dict['text'] = fhir_str
    fhir_resource_dict['name'] = fhir_resource_name
    return fhir_resource_dict
